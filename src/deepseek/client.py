...
class Client:
    ...
    def upload_data(self, filename: str, purpose='assistants') -> Result:
        ...
        with open(filename, 'rb') as file:
            try:
                ...
                self.uploaded_files.append(dict(
                    filename=filename,
                    purpose=purpose,
                    id=response.id,
                ...
                print_msg(f"{filename}: File uploaded successfully! (size: {response.bytes})")
                return Result(True, None, response)
            except FileNotFoundError:
                return Result(False, f"Nonexistent file: {filename}", None)
            except APIConnectionError:
                return Result(
                    False,
                    f"API connection error. Please restart client",
                    APIConnectionError
                )
            except APIError as e:
                return Result(False, f"API Error: {e}", e)
    ...
    def ask(
        ...
        if out := self.cache.get(question):
            if clipboard: write_clip(out)
            if stdout: print(out)
            return Result(True, None, out)
        ...
        if images:
            [content.append(self.input_image(i)) for i in tolist(images)]

        # temporary workaround for client.files api 
        if files:
            # TODO: Make sure files are data files according to deepseek 
            [content.append(self.input_data(f)) for f in tolist(files)]
        ...
        try:
            ...
            response_stream = Stream(client.chat.completions.create(
                model=model,
                messages=messages,
                stream=stream,
            ...
            return Result(False, "KeyboardInterrupt", KeyboardInterrupt)
        except EOFError:
            return Result(False, "EOF Error", EOFError)
        except APIConnectionError:
            return Result(False, "Connection Error. Please restart client", APIConnectionError)
        except APIError:
            return Result(False, "API Error", APIError)
